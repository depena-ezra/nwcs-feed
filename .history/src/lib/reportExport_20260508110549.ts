...
const SCHOOL_NAME = "Nasugbu West Central School";
const SCHOOL_ID = "107512";
...
export function exportAttendanceToPDF(report: any, filters: any, download = true) {
  ...
  const lastY = lastAutoTable?.finalY ?? 0;
  if (lastY < 250 && lastAutoTable) {
    doc.text("Student Attendance Summary", 20, lastAutoTable.finalY + 15);
    ...
    autoTable(doc, {
      ...
      body: studentData,
      startY: (lastAutoTable?.finalY ?? 0) + 20,
...
// DOCX exports
function buildDocxTitleBlock(args: { title: string; filters: any; final?: boolean }) {
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            text: `Attendance Present Rate: ${report.attendance.presentRatePct.toFixed(1)}%`,
          }),
          new Paragraph({
            text: `Meals Received Rate: ${report.attendance.mealsReceivedRatePct.toFixed(1)}%`,
          }),
          new Paragraph({
