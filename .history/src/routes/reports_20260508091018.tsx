...
import {
  ...
  exportMasterlistToWord,
  exportMeasurementToWord,
  exportAttendanceToWord,
  exportImprovementToWord,
  exportConsolidatedToWord,
  ...
  return (
    <AppLayout title="Reports" subtitle="Generate official and consolidated reports">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button onClick={handlePreviewReport} className="gap-2">
                <Eye className="h-4 w-4" /> Preview Report (Draft)
              </Button>
      ...
      <AlertDialog open={confirmFinalize} onOpenChange={setConfirmFinalize}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Finalize and export as official report?</AlertDialogTitle>
            <AlertDialogDescription>
              Preview generates draft files for review. Finalize locks the report as official and exports
              “FINAL” versions (Word/Excel/PDF) with an OFFICIAL label.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                // Final mode: exports include an OFFICIAL label/watermark.
                if (data.type === "masterlist") {
                  exportMasterlistToWord(data.rows, filters, true);
                  exportMasterlistToExcel(data.rows, filters, true);
                  exportMasterlistToPDF(data.rows, filters, true);
                } else if (data.type === "baseline" || data.type === "endline") {
                  exportMeasurementToWord(data.rows, data.type, filters, true);
                  exportMeasurementToExcel(data.rows, data.type, filters, true);
                  exportMeasurementToPDF(data.rows, data.type, filters, true);
                } else if (data.type === "attendance") {
                  exportAttendanceToWord(data.report, filters, true);
                  exportAttendanceToExcel(data.report, filters, true);
                  exportAttendanceToPDF(data.report, filters, true);
                } else if (data.type === "improvement") {
                  exportImprovementToWord(data.report, filters, true);
                  exportImprovementToExcel(data.report, filters, true);
                  exportImprovementToPDF(data.report, filters, true);
                } else if (data.type === "consolidated") {
                  exportConsolidatedToWord(data.report, filters, true);
                  exportConsolidatedToExcel(data.report, filters, true);
                  exportConsolidatedToPDF(data.report, filters, true);
                }

                toast.success("Final report exported (OFFICIAL)");
              }}
            >
              Finalize
            </AlertDialogAction>
