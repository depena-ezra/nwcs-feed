...
const statusRank: Record<NutritionStatus, number> = {
  Underweight: 0,
  "At Risk": 1,
  Normal: 2,
  Overweight: 1.5, // treat overweight as not strictly"worse"; keep comparison conservative.
};

function improved(b: NutritionStatus, e: NutritionStatus) {
  // Simple: compare ranks; higher is "better".
  return statusRank[e] > statusRank[b];
}

export function buildNutritionalImprovement(filters: ReportFilters): NutritionalImprovementReport {
  const eligible = filterStudents(filters);
  const baselineCounts: Record<NutritionStatus, number> = {
    Normal: 0,
    Underweight: 0,
    Overweight: 0,
    "At Risk": 0,
  };
  const endlineCounts: Record<NutritionStatus, number> = {
    Normal: 0,
    Underweight: 0,
    Overweight: 0,
    "At Risk": 0,
  ...
  const rows = eligible
    .map((s) => {
      baselineCounts[s.baselineStatus]++;
      endlineCounts[s.endlineStatus]++;
      const bmiDelta = +(s.endlineBMI - s.baselineBMI).toFixed(2);
      if (improved(s.baselineStatus, s.endlineStatus)) improvedCount++;
      return {
        studentId: s.id,
        name: s.name,
        grade: s.grade,
        ...
        endlineStatus: s.endlineStatus,
        baselineBMI: s.baselineBMI,
        endlineBMI: s.endlineBMI,
        bmiDelta,
      };
    })
    .sort((a, b) => a.name.localeCompare(b.name));

  const allStatuses: NutritionStatus[] = ["Underweight", "At Risk", "Normal", "Overweight"];

  const baseline = allStatuses.map((st) => ({ status: st, count: baselineCounts[st] }));
  const endline = allStatuses.map((st) => ({ status: st, count: endlineCounts[st] }));

  const totalStudents = eligible.length;
  const improvedRatePct = totalStudents ? (improvedCount / totalStudents) * 100 : 0;
...
export interface ConsolidatedSchoolFeedingReport {
...
export function buildConsolidatedSchoolFeedingReport(filters: ReportFilters): ConsolidatedSchoolFeedingReport {
  ...
  // Expected as number of records per student-day (mock is 1 record per student per day)
  const dayStudentSet = new Set<string>();
  for (const r of filtered) {
    dayStudentSet.add(`${r.date}::${r.studentId}`);
    if (r.present) totalPresent++;
    if (r.receivedMeal) totalMealsReceived++;
  }
  totalExpected = dayStudentSet.size;

  const presentRatePct = totalExpected ? (totalPresent / totalExpected) * 100 : 0;
  const mealsReceivedRatePct = totalExpected ? (totalMealsReceived / totalExpected) * 100 : 0;

  const attendanceReport = buildFeedingAttendanceSummary(filters);

  const improvement = buildNutritionalImprovement(filters);

  return {
    filters,
    totalBeneficiaries,
    attendance: {
      daysCount: attendanceReport.days.length,
      totalPresent,
      totalExpected,
      presentRatePct,
      totalMealsReceived,
      mealsReceivedRatePct,
    },
    nutritionalImprovement: {
      baseline: improvement.baseline,
      endline: improvement.endline,
      improvedCount: improvement.improvedCount,
      improvedRatePct: improvement.improvedRatePct,
    },
  };
}
