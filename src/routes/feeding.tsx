...
export const Route = createFileRoute("/feeding")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Daily feeding attendance and meal distribution monitoring.",
      },
    ],
  }),
...
function FeedingPage() {
  ...
  const save = () => {
    toast.success("Monitoring saved as draft");
    setDirty(false);
  };
  const submit = () => {
                    ...
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-center">
                        <Checkbox
                          ...
                          checked={r.present}
                          onCheckedChange={(v) =>
                            update(r.id, { present: !!v, absent: v ? false : r.absent })
                          }
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <Checkbox
                          ...
                          onCheckedChange={(v) =>
                            update(r.id, {
                              absent: !!v,
                              present: v ? false : r.present,
                              received: v ? false : r.received,
                            })
                          }
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <Checkbox
                          className="h-5 w-5"
                          checked={r.received}
                          onCheckedChange={(v) => update(r.id, { received: !!v })}
                          disabled={r.absent}
                        />
                      </TableCell>
                      <TableCell>
          ...
          <div className="sticky bottom-0 z-10 mt-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border bg-card/95 px-4 py-3 shadow-lg backdrop-blur">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button onClick={submit} className="gap-2">
                <Send className="h-4 w-4" /> Submit Daily Record
              </Button>
            </div>
          </div>
        </>
